...
这个工具用于在浏览器中基于一个固定的 `index.html` 母版，结合软件名称、版本号、25 个可配置视觉维度、9 条强制要求，自动生成新的 `index.html`，并支持预览、重新生成、复制源码和下载。
...
9. 支持复制源码。
10. 支持下载 `index.html`。
11. 生成后必须运行 validator 校验 9 条硬性要求。
...
生成结果必须遵守 `docs/硬性要求.txt` 中的 9 条规则，尤其是：
