...
【执行优先级】
硬性要求 > 本次明确要求 > 实体维度 > 原始HTML默认样式 > 你的设计判断
...
5. ${input.refining ? '最终HTML必须与上一版产生和本次要求对应的实质差异，禁止无修改原样返回。' : '最终HTML必须体现系统业务名称、业务菜单和启用维度。'}
...
2. 必须以输入的完整HTML为基础精准修改，不得脱离母版重新设计。
...
8. 实体维度是默认设计约束；用户本次明确要求可覆盖维度默认值，但不得违反硬性要求。
...
12. 不得删除或改名硬性要求依赖的ID、函数和核心变量。
13. 功能型维度必须实现完整行为，不能只生成视觉图标。用户信息维度包含“下拉”时，必须生成可点击触发器、下拉菜单、“修改密码”“退出登录”、重复点击关闭及点击页面其他区域关闭逻辑。
14. 输出前必须逐字重读本次要求，并在最终代码中找到对应实现证据；找不到时继续修改，不得结束输出。
15. 输出前检查HTML标签闭合、CSS、JavaScript语法及全部硬性要求。
16. 不要解释、不要总结、不要输出Markdown代码块、不要返回JSON。
17. 回复必须从<!DOCTYPE html>开始，以</html>结束，只能包含完整index.html。`;

export function buildRepairPrompt(html: string, errors: string[]) {
