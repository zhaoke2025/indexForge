...
export default function GenerateIndexPage({ initialRecord, hasCurrentTemplate, onGenerated }: Props) {
  ...
  return <div className="grid grid-cols-[400px_minmax(0,1fr)] gap-5 p-5">
    <section className="space-y-4">
      ...
      {record && <>
        ...
        {record.dimensions.length > 0 ? <div className="rounded border border-slate-200 bg-white p-4"><div className="text-sm font-semibold">维度决策（{record.dimensions.length}）</div><div className="mt-2 max-h-48 space-y-2 overflow-auto">{record.dimensions.map((item) => <div className="rounded bg-slate-50 px-3 py-2 text-xs" key={item.dimensionId}><b>{item.dimensionId}</b>：{String(item.value)}<div className="mt-1 text-slate-500">{item.reason}</div></div>)}</div></div> : <div className="rounded border border-slate-200 bg-white p-4 text-sm text-slate-600">本次采用AI深度生成：模型直接读取母版、启用维度和硬性要求，并输出完整HTML。</div>}
