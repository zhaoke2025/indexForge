...
import type { HistoryRecord, LoginHistoryRecord, MenuItemConfig, TemplateState } from './core/types';
...
type PageKey = 'generate' | 'login' | 'loginDimensions' | 'loginRequirements' | 'dimensions' | 'requirements' | 'template' | 'menu' | 'history';
const navItems = [
  ...
  { key: 'template', name: '母版管理', icon: FileCode2 }, { key: 'menu', name: '菜单配置', icon: Menu }, { key: 'history', name: '生成记录', icon: History },
...
const pageMeta: Record<PageKey, { title: string; desc: string }> = {
  ...
  template: { title: '母版管理', desc: '上传、检测和选择当前HTML母版' }, menu: { title: '菜单配置', desc: '手动菜单配置工具' }, history: { title: '生成记录', desc: '查看服务器保存的生成和调整记录' },
};
...
export default function App() {
  ...
  useEffect(() => { reload(); }, [reload]); useEffect(() => { writeStorage('indexforge.loginConfig', loginConfig); }, [loginConfig]); useEffect(() => { writeStorage('indexforge.menuConfig', menuConfig); }, [menuConfig]);
  ...
  }, [activePage, selectedRecord, selectedLoginRecord, loginConfig, dimensions, loginDimensions, loginRequirements, presetId, requirements, templates, menuConfig, history, loginHistory, reload]);
